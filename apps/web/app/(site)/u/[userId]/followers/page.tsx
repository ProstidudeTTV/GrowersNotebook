    ...
    <SitePageShell>
      <main className="mx-auto max-w-2xl py-8">
        ...
        <p className="mt-1 text-sm text-[var(--gn-text-muted)]">
          {list.hidden
            ? isOwner
              ? "Your follower list is private — only you can see it."
              : "This grower keeps their follower list private."
            : `${list.total.toLocaleString()} grower${list.total === 1 ? "" : "s"} follow ${profileName} on ${SITE_NAME}.`}
        </p>

        {loadFailed ? (
          <p className="mt-10 text-center text-sm text-[var(--gn-text-muted)]">
            Could not load followers right now. Try again later.
          </p>
